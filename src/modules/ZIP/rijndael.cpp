...
#else
  for (int I=0;I<16;I++)
    ((uint8_t*)dest)[I]=((uint8_t*)arg1)[I]^((uint8_t*)arg2)[I];
#endif
}


inline void Xor128(uint8_t *dest,const uint8_t *arg1,const uint8_t *arg2,
                   const uint8_t *arg3,const uint8_t *arg4)
{
#if defined(PRESENT_INT32) && defined(ALLOW_MISALIGNED)
...
inline void Copy128(uint8_t *dest,const uint8_t *src)
{
#if defined(PRESENT_INT32) && defined(ALLOW_MISALIGNED)
...
void Rijndael::Init(bool Encrypt,const uint8_t *key,unsigned int keyLen,const uint8_t * initVector)
{
  unsigned int uKeyLenInuint8_ts;
  switch(keyLen)
  {
    case 128:
      uKeyLenInuint8_ts = 16;
      m_uRounds = 10;
      break;
    case 192:
      uKeyLenInuint8_ts = 24;
      m_uRounds = 12;
      break;
    case 256:
      uKeyLenInuint8_ts = 32;
      m_uRounds = 14;
      break;
  }
...
void Rijndael::blockDecrypt(const uint8_t *input, size_t inputLen, uint8_t *outBuffer)
{
  if (inputLen <= 0)
    return;
...
void Rijndael::keyEncToDec()
{
  ...
  {
    uint8_t n_expandedKey[4][4];
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 4; j++)
      {
        uint8_t *w=m_expandedKey[r][j];
...
#define fwd_affine(x) \
    (w = (unsigned int)x, w ^= (w<<1)^(w<<2)^(w<<3)^(w<<4), (uint8_t)(0x63^(w^(w>>8))))

#define inv_affine(x) \
    (w = (unsigned int)x, w = (w<<1)^(w<<3)^(w<<6), (uint8_t)(0x05^(w^(w>>8))))
...
void Rijndael::GenerateTables()
{
  ...
  for(int i = 0; i < 256; ++i)
  {   
    unsigned char b=S[i]=fwd_affine(FFinv((uint8_t)i));
    ...
    S5[i] = b = FFinv(inv_affine((uint8_t)i));
